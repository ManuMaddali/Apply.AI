...
import { BulkProcessingGuard } from './UsageLimitGuard'
...
const JobUrlsInput = ({ jobUrls, onJobUrlsChange }) => {
  ...
  return (
    <BulkProcessingGuard jobCount={urlCount}>
        ...
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-gray-600 mt-1">
                Add URLs for positions you&apos;re interested in
                {!isProUser && urlCount > 1 && (
                  <span className="block text-yellow-600 font-medium mt-1">
                    ⚠️ Free plan: Only first job will be processed
                  </span>
                )}
              </p>
            </div>
          ...
          {urlCount > 0 && (
            <div className="flex items-center gap-2">
              <div className={`text-sm font-semibold px-4 py-2 rounded-full border ${
                !isProUser && urlCount > 1 
                  ? 'text-yellow-700 bg-gradient-to-r from-yellow-100 to-orange-100 border-yellow-200'
                  : 'text-purple-600 bg-gradient-to-r from-purple-100 to-pink-100 border-purple-200'
              }`}>
                {urlCount} {urlCount === 1 ? 'position' : 'positions'}
                {!isProUser && urlCount > 1 && (
                  <span className="ml-1 text-xs">(1 will process)</span>
                )}
              </div>
            </div>
          )}
...
${isProUser ? '• Support for up to 10 positions at once' : '• Free plan: Process one job at a time (upgrade for bulk processing)'}`}
          rows={8}
        ...
        {/* Floating counter */}
        <div className={`absolute top-4 right-4 backdrop-light rounded-lg px-3 py-1 text-xs font-medium border ${
          !isProUser && urlCount > 1
            ? 'bg-yellow-50/90 text-yellow-700 border-yellow-200'
            : 'bg-white/90 text-gray-600 border-gray-200'
        }`}>
          {urlCount}/{isProUser ? '10' : '1 (Free)'}
        </div>
      </div>
